...
//         className="min-h-screen bg-brand-bg flex items-center justify-center px-6"
//       >
...
//             onClick={() => navigate("/")}
//             className="mt-6 text-xs text-zinc-500 hover:text-zinc-300 flex items-center gap-1 mx-auto transition-colors"
//           >
...
//           <TabsList
//             className="bg-brand-surface border border-white/10 mb-8"
//             data-testid="admin-tabs"
//           >
//             <TabsTrigger
//               value="cars"
//               data-testid="tab-cars"
//               className="data-[state=active]:bg-brand-gold data-[state=active]:text-black gap-2 text-xs uppercase tracking-wider"
//             >
...
//             <CarsManager token={token} />
//           </TabsContent>
//           <TabsContent value="articles">
//             <ArticlesManager token={token} />
//           </TabsContent>
...
//                 {editCar ? "Редактировать" : "Добавить автомобиль"}
//               </DialogTitle>
//             </DialogHeader>
//             <CarForm
//               token={token}
...
//                 Тип
//               </TableHead>
//               <TableHead className="text-xs uppercase tracking-wider text-zinc-500 text-right">
//                 Действия
//               </TableHead>
//             </TableRow>
//           </TableHeader>
...
//                       {car.is_from_europe && (
//                         <span className="text-brand-gold text-[10px] font-semibold uppercase bg-brand-gold/10 px-1.5 py-0.5">
//                           EU
//                         </span>
//                       )}
//                       {car.is_commission && (
//                         <span className="text-brand-blue text-[10px] font-semibold uppercase bg-brand-blue/10 px-1.5 py-0.5">
//                           КОМ
//                         </span>
//                       )}
//                     </div>
...
//         ? form.images.filter(Boolean).slice(0, 3)
//         : [];
//       const payload = {
//         ...form,
//         images,
...
//       if (car) {
//         await api.updateCar(car.id, payload, token);
//       } else {
...
//         <Select
//           value={customMode ? CUSTOM_OPTION : value || undefined}
//           onValueChange={handleSelectChange}
//         >
//           <SelectTrigger
//             className="bg-transparent border-white/20 text-white h-11"
//             data-testid={testId}
//           >
...
//             <SelectItem value={CUSTOM_OPTION}>Свой вариант</SelectItem>
//           </SelectContent>
//         </Select>
...
//           testId="car-brand"
//           customInputTestId="car-brand-custom"
//           customPlaceholder="Введите марку"
//           required
//         />
...
//           testId="car-fuel"
//           customInputTestId="car-fuel-custom"
//           customPlaceholder="Введите тип топлива"
//         />
...
//           testId="car-body"
//           customInputTestId="car-body-custom"
...
//           testId="car-drive"
//           customInputTestId="car-drive-custom"
//           customPlaceholder="Введите тип привода"
//         />
...
//           }
//           onChange={(urls) => {
//             const images = Array.isArray(urls) ? urls.slice(0, 3) : [];
//             setForm({
//               ...form,
//               images,
//               image_url: images[0] || "",
...
//           data-testid="car-europe-toggle"
//         >
//           <input
//             type="checkbox"
//             checked={form.is_from_europe}
//             onChange={(e) => update("is_from_europe", e.target.checked)}
//             className="w-4 h-4 accent-[#FFC631]"
//           />
...
//         <label
//           className="flex items-center gap-2 cursor-pointer"
//           data-testid="car-commission-toggle"
//         >
//           <input
//             type="checkbox"
//             checked={form.is_commission}
//             onChange={(e) => update("is_commission", e.target.checked)}
//             className="w-4 h-4 accent-[#1A5697]"
//           />
...
//     try {
//       setArticles(await api.getArticles(true));
//     } catch (e) {
//       console.error(e);
//     } finally {
//       setLoading(false);
...
//     try {
//       if (article) {
//         await api.updateArticle(article.id, form, token);
//       } else {
//         await api.createArticle(form, token);
//       }
//       onDone();
//     } catch (e) {
//       console.error(e);
...
//           className="input-dark text-sm resize-none"
//           required
//           placeholder="Текст статьи. Используйте переносы строк для абзацев, '- ' для списков."
//         />
//       </div>
//       <label
//         className="flex items-center gap-2 cursor-pointer"
//         data-testid="article-published-toggle"
//       >
//         <input
//           type="checkbox"
...
//     try {
//       setSubs(await api.getSubmissions(token));
//     } catch (e) {
//       console.error(e);
//     } finally {
//       setLoading(false);
...
//           Заявки ({subs.length})
//           {unreadCount > 0 && (
//             <span className="ml-2 text-xs bg-brand-gold text-black px-2 py-0.5 font-semibold">
//               {unreadCount} новых
//             </span>
//           )}
//         </h2>
...
//                 <TableRow
//                   key={sub.id}
//                   className={`border-white/10 ${!sub.is_read ? "bg-brand-gold/5" : ""}`}
//                   data-testid={`sub-row-${sub.id}`}
//                 >
//                   <TableCell className="font-medium">
//                     {!sub.is_read && (
//                       <span className="inline-block w-2 h-2 bg-brand-gold rounded-full mr-2" />
//                     )}
//                     {sub.name}
//                   </TableCell>
...
//                     </span>
//                   </TableCell>
//                   <TableCell className="max-w-[200px] truncate text-zinc-400 text-sm">
//                     {sub.message || "—"}
//                   </TableCell>
//                   <TableCell className="text-sm text-zinc-500">
...
//                       {!sub.is_read && (
//                         <button
//                           data-testid={`read-sub-${sub.id}`}
//                           onClick={() => handleRead(sub.id)}
...
const numberInputClassName =
  "input-dark text-sm [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none";
...
const SelectWithCustom = memo(function SelectWithCustom({
  label,
  value,
  onChange,
  options,
...
function CarForm({ token, car, onDone }) {
  const [form, setForm] = useState(() =>
    car
      ? {
          ...car,
          year: normalizeNumberValue(car.year),
          price: normalizeNumberValue(car.price),
          mileage: normalizeNumberValue(car.mileage),
          images:
  ...
  const updateNumber = useCallback(
    (key, value) => {
      update(key, onlyDigits(value));
    },
    [update],
  );

  const updateBrand = useCallback((value) => update("brand", value), [update]);
  const updateTransmission = useCallback(
    (value) => update("transmission", value),
    [update],
  );
  const updateFuelType = useCallback(
    (value) => update("fuel_type", value),
    [update],
  );
  const updateBodyType = useCallback(
    (value) => update("body_type", value),
    [update],
  );
  const updateDriveType = useCallback(
    (value) => update("drive_type", value),
    [update],
  );
  ...
  const handleSave = async (e) => {
    ...
    const safeImages = Array.isArray(form.images)
      ? form.images.slice(0, 3)
      : [];
    ...
    <form onSubmit={handleSave} className="space-y-4" data-testid="car-form">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div>
          ...
          <input
            ...
            inputMode="numeric"
            value={form.year}
            onChange={(e) => updateNumber("year", e.target.value)}
        ...
        <div>
          ...
          <input
            ...
            inputMode="numeric"
            value={form.mileage}
            onChange={(e) => updateNumber("mileage", e.target.value)}
            className={numberInputClassName}
            placeholder="120000"
          />
        </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <Select value={form.transmission} onValueChange={updateTransmission}>
            <SelectTrigger
              className="bg-transparent border-white/20 text-white h-11"
              data-testid="car-transmission"
            >
      ...
      <div>
        ...
        <p className="text-[11px] text-zinc-500 mt-2">
          Можно загрузить до 3 фотографий. Первая фотография будет основной в
          каталоге.
        </p>
      </div>
